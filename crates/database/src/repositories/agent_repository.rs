...
impl AgentRepository for PostgresAgentRepository {
    ...
    async fn get_instance(&self, instance_id: Uuid) -> anyhow::Result<Option<AgentInstance>> {
        ...
        let row = client
            .query_opt(
                "SELECT id, user_id, instance_id, name, type, public_ssh_key, instance_url, instance_token, dashboard_url, agent_api_base_url, status, created_at, updated_at
    ...
    async fn get_instance_usage(
            ...
            client
                .query(
                    "SELECT u.id, u.user_id, u.instance_id, u.api_key_id, k.name,
                            ...
                            COALESCE((u.details->>'cache_read_tokens')::BIGINT, 0),
        ...
        let usage = rows
            ...
            .map(|r| UsageLogEntry {
                ...
                quantity: r.get(10),
                input_cost: r.get(11),
                output_cost: r.get(12),
                total_cost: r.get(13),
                model_id: r.get::<_, Option<String>>(14).unwrap_or_default(),
                request_type: r.get(15),
                created_at: r.get(16),
            })
            .collect();
    ...
    async fn get_user_passkey_credentials(
        ...
        let row = client
            .query_opt(
                "SELECT auth_secret, backup_passphrase FROM user_passkey_credentials WHERE user_id = $1",
                &[&user_id],
            )
            .await?;
    ...
    async fn get_crabshack_instance_name(
        ...
        let row = client
            .query_opt(
                "SELECT crabshack_instance_name FROM agent_instances WHERE id = $1",
                &[&instance_id],
            )
            .await?;
    ...
    async fn admin_update_instance(
        ...
        if let Some(ref val) = crabshack_instance_name {
            set_clauses.push(format!("crabshack_instance_name = ${}", param_idx));
            param_values.push(Box::new(val.clone()));
            param_idx += 1;
        }
