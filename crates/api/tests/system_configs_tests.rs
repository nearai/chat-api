...
#[tokio::test]
#[serial(write_system_configs)]
async fn test_agent_hosting_not_visible_in_public_config() {
    let server = create_test_server().await;

    let non_admin_email = "test_user_agent_hosting_public@no-admin.org";
    let non_admin_token = mock_login(&server, non_admin_email).await;

    let admin_email = "test_admin_agent_hosting_public@admin.org";
    let admin_token = mock_login(&server, admin_email).await;

    // Admin sets an agent_hosting override
    let update_body =
        json!({ "agent_hosting": { "crabshack": { "ironclaw_image": "img:visibility" } } });
    let response = server
        .patch("/v1/admin/configs")
        .add_header(
            http::HeaderName::from_static("authorization"),
    ...
    // Non-admin GET /v1/configs should NOT show agent_hosting
    let response = server
        .get("/v1/configs")
        .add_header(
            http::HeaderName::from_static("authorization"),
    ...
    assert!(
        body.get("agent_hosting")
            .and_then(|cfg| cfg.get("crabshack"))
            .is_none(),
        "Public config should NOT expose agent_hosting"
    );

    // Admin GET /v1/admin/configs SHOULD show agent_hosting
    let response = server
        .get("/v1/admin/configs")
        .add_header(
            http::HeaderName::from_static("authorization"),
    ...
    assert_eq!(
        body.get("agent_hosting")
            .and_then(|cfg| cfg.get("crabshack"))
            .and_then(|c| c.get("ironclaw_image")),
        Some(&json!("img:visibility")),
