...
pub fn create_router_with_cors(app_state: AppState, cors_config: config::CorsConfig) -> Router {
    ...
    // Referral routes (requires user authentication)
    let referral_routes = referrals::create_referrals_router().layer(from_fn_with_state(
        auth_state.clone(),
        crate::middleware::auth_middleware,
    ));
    ...
    let router = Router::new()
        ...
        .merge(referral_routes) // Referral routes (requires user auth)
